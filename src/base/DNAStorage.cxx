...
void DNAStorage::store_DNA_group(PT(PandaNode) node, PT(DNAGroup) group) {
    m_dna_groups[node] = group;
}

PT(DNAGroup) DNAStorage::find_DNA_group(PT(PandaNode) node) {
    for (group_vec_t::iterator it = m_dna_groups.begin(); it != m_dna_groups.end(); ++it) {
        if (it->first == node) {
            return it->second;
        }
    }
...
PT(PandaNode) DNAStorage::find_panda_node(PT(DNAGroup) group) {
    for (group_vec_t::iterator it = m_dna_groups.begin(); it != m_dna_groups.end(); ++it) {
        if (it->second == group) {
            return it->first;
        }
...
void DNAStorage::write(std::ostream& out, unsigned int nbyte) {
    for (suit_point_vec_t::iterator it = m_suit_points.begin(); it != m_suit_points.end(); ++it) {
        (*it)->write(out, this, nbyte);
    }
    for (group_vec_t::iterator it = m_dna_groups.begin(); it != m_dna_groups.end(); ++it) {
        (*it).second->write(out, this, nbyte);
    }
}
