...
typedef std::map<PT(PandaNode), PT(DNAGroup)> group_vec_t;
...
class EXPCL_DNA DNAStorage
{
    PUBLISHED:
        ...
        void store_DNA_group(PT(PandaNode) node, PT(DNAGroup) group);
        PT(DNAGroup) find_DNA_group(PT(PandaNode) node);
        PT(PandaNode) find_panda_node(PT(DNAGroup) group);
        ...
        virtual void write(std::ostream& out, unsigned int nbyte = 0);
